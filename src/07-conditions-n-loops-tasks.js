...
function doRectanglesOverlap(rect1, rect2) {
  const top = rect2.top < rect1.top + rect1.height;
  const left = rect2.left < rect1.left + rect1.width;
...
function isInsideCircle(circle, point) {
  const coordinateX = point.x - circle.center.x;
  const coordinateY = point.y - circle.center.y;
  const distanceSquared = coordinateX ** 2 + coordinateY ** 2;
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < arr.length - 1; i += 1) {
    if (bracketPairs.some((el) => el[0] === arr[i] && el[1] === arr[i + 1])) {
      arr.splice(i, 2);
      i -= 2;
    }
  }
...
function getCommonDirectoryPath(pathes) {
  const commonPath = pathes.reduce((result, path) => {
    let i = 0;
    while (result[i] === path[i] && i < path.length) {
      i += 1;
    }
    return result.slice(0, i);
  });

  const commonDir = commonPath.slice(0, commonPath.lastIndexOf('/') + 1);
  return commonDir;
}
